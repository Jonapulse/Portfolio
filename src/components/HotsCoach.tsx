...
export const HotsCoach: React.VFC = () => (
  <div>
	...
	<p className="articleParagraph">While playing a particula MOBA, my friends were faced with a terrible dilemma. Each of us had a character we loved to play but were disastrously unskilled at, however we enjoyed winning a game every once in a while.</p>
	<p className="articleParagraph">My solution was to build a tool to collect player data and recommend playstyles. Using Python and sourcing data from <a className="emailLink" href="https://hotslogs.com">an online, player-run repository</a>, the tool would crawl through the site (using the <a className="emailLink" href="https://www.crummy.com/software/BeautifulSoup/bs4/doc/">Beautiful Soup python library</a>. Starting from a player ID, the system would collect their personal winrates, the winrates of friends with sufficient shared games, and then the general champion winrate information, including "duo winrates" or data on relative success when different champions played together.</p>
	...
	<p className="articleParagraph">The fix was filter out data with insufficient values included, substituting data where the system required it (the default duo winrate assumed two champions had no effect on eachother). The filter was also applied to champions for which players had very few games. </p>
