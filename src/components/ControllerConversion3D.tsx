...
export const ControllerConversion3D: React.VFC = () => (
  <div className="articleContent">
	  ...
	  <div className="articlePictureRight"><HoverGifArticle stillPath="./art/cubeldaIssues.jpg" gifPath="./art/cubeldaIssues.gif" subTitle="Bad descent followed by a strange hover"> </HoverGifArticle></div>
	  <p className="articleParagraph">Later, when I began work on a 3D Platforming project, early experiments confirmed that I may face similar problems with the built-in physics. Again, the content of this game was not focused on much of the functionality an “all-in-one” physics solution might provide, so I decided to adapt the Raycasting controller that had served me well in my previous project. The raycasting solution was no swiss-army knife, but it had worked well, I was familiar with it, and I believed thatadapting to 3D would be successful.</p>
	  ...
	  <p className="articleParagraph">Ultimately though, I arrived at a simpler concept that delivered a working solution. 3D movement could be reduced to 2D movement in the plane defined using the player's vertical orientation (their "up") and their movement vector orthogonal to that orientation. Essentially, the player would be moving and calculating collision in a world defined by the 2D slice that this plane defines. Applying that naively would treat the player as infinitely thin, but it was simple to with parallel "slices" encompassing the player’s intended width. </p>
	  <div className="articlePictureCenter"><HoverGifArticle stillPath="./art/sliceOf3D.jpg" gifPath="./art/sliceOf3D.gif" subTitle="Navigating 3D with a 2D enviromental slice"> </HoverGifArticle></div>
	  ...
	  <p className="articleParagraph">The resulting system has a number of shortfalls resulting from its simplicity. It cannot resolve collisions with moving objects. When the player is stationary, to attempt is made to resolve collisions. And the system does not simulate physics — your player has no mass.  But it was made for a simple game that didn’t need any of that, yet which had complexities that were not served by the available tools.</p>
  </div>
);
